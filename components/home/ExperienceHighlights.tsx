    ...
    <section className="py-24 relative overflow-hidden px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-4xl mx-auto space-y-16">
          ...
          {highlightedItems.map((item: any, index: number) => {
            ...
            return (
              <motion.div
                ...
                className={`relative flex flex-col sm:flex-row items-center justify-between gap-8 ${isEven ? 'sm:flex-row' : 'sm:flex-row-reverse'}`}
              >
                {/* Content Card */}
                <div className={`w-full sm:w-[45%] p-6 rounded-3xl bg-white/[0.03] border border-white/10 backdrop-blur-xl hover:border-orange-500/30 transition-colors group shadow-xl ${isEven ? 'text-left' : 'sm:text-right'}`}>
                   <div className={`flex items-center gap-3 mb-4 ${isEven ? '' : 'sm:flex-row-reverse'}`}>
                      <div className="p-2.5 rounded-xl bg-orange-500/10 border border-orange-500/20 text-orange-500 group-hover:scale-110 transition-transform">
                         <Building2 className="w-4 h-4" />
                      </div>
                      <div>
                        <h3 className="font-bold text-lg leading-none">{item.company}</h3>
                        <p className="text-xs text-orange-500/70 font-bold mt-1 uppercase tracking-wider">{item.role}</p>
                      </div>
                   </div>
                   ...
                   <div className={`flex items-center gap-4 mt-6 pt-4 border-t border-white/5 ${isEven ? 'justify-start' : 'sm:justify-end justify-start'}`}>
                ...
                {/* Center Node */}
                <div className="absolute left-0 sm:left-1/2 top-6 sm:top-1/2 w-4 h-4 sm:-translate-x-1/2 sm:-translate-y-1/2 z-10 hidden sm:block">
        ...
        <div className="flex justify-center pt-8">
            <Link href="/experience" className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-orange-500/10 text-orange-500 font-bold hover:bg-orange-500 hover:text-white transition-all border border-orange-500/20 group">
