...
function LogoWithAnimation() {
    ...
    useEffect(() => {
        ...
        window.addEventListener("restore-bb8-logo", handleRestore);
        return () => window.removeEventListener("restore-bb8-logo", handleRestore);
    }, []);

    const handleSummon = () => {
        if (isSummoned || isExiting) return;
        setIsExiting(true);
        // Wait for roll-away animation to complete before signaling the companion
        setTimeout(() => {
            setIsSummoned(true);
            window.dispatchEvent(new CustomEvent("summon-bb8"));
        }, 800);
    };
    ...
    return (
        <div className="flex items-center space-x-2.5 group">
            <Link href="/" onClick={(e) => {
                // If clicking the BB-8 SVG specifically, summon the companion
                // This allows clicking the text ("Fernando Maximo") for normal navigation
            }} className="flex items-center space-x-2.5">
                <AnimatePresence mode="wait">
                    {!isSummoned && (
                        <motion.div
                            key="bb8-logo"
                            initial={{ x: -200, rotate: -720, opacity: 0 }}
                            animate={isExiting 
                                ? { x: -400, rotate: -1080, opacity: 0 } 
                                : { x: 0, rotate: 0, opacity: 1 }
                            }
                            exit={{ x: -400, rotate: -1080, opacity: 0 }}
                            transition={{ 
                                type: "spring", 
                                stiffness: 150, 
                                damping: 20,
                ...
                <div className="flex flex-col">
                    ...
                    >
                        Fernando{" "}
                        <span className="text-orange-500">Máximo</span>
                    </motion.span>
                </div>
            </Link>
        </div>
                        ...
                        >
                            <Link
                                ...
                                className={`relative px-4 py-2 rounded-xl transition-all duration-200 ${
                                    pathname === link.href
                                        ? "text-orange-500 font-semibold bg-orange-500/8"
                            ...
                            >
                                ...
                                {pathname === link.href && (
                                    <motion.span
                                        layoutId="nav-active"
                                        className="absolute inset-0 rounded-xl ring-1 ring-orange-500/30 bg-orange-500/8"
                ...
                <div className="flex-1 flex justify-end items-center space-x-2">
                    <div className="hidden md:flex items-center space-x-2">
                        <motion.a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="p-2 rounded-xl text-foreground/60 hover:text-foreground hover:bg-white/5 transition-all"
                        >
                        ...
                        <motion.button
                            ...
                            className="relative rounded-xl p-2 hover:bg-white/5 transition-all w-9 h-9 flex items-center justify-center text-foreground/60 hover:text-foreground"
                        >
